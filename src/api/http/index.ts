...
import { TIMEOUT, IMiddlewareParams, IRefreshTokenParams, IRefreshTokenResponse, IAPIConfig } from './types';
...
class HttpService {
  ...
  public getToken() {
    return this.token || localStorage.getItem(this.config?.accessTokenStorageKey || 'accessToken');
  }
        ...
        try {
          ...
          if (statusCode !== 401) reject(e);
            ...
            if (!authRefreshToken && !anonymousRefreshToken) reject(e);
              ...
              try {
                const { accessToken, refreshToken } = await this.refreshToken({
                  currentRefreshToken: anonymousRefreshToken,
                });
                localStorage.setItem(this.config?.accessTokenStorageKey || '', accessToken);
                localStorage.setItem(this.config?.refreshTokenStorageKey || '', refreshToken);
                this.setToken(accessToken);
  ...
  public async refreshToken({ currentRefreshToken }: IRefreshTokenParams): Promise<IRefreshTokenResponse> {
    try {
      const res = await axios.post(
        `${this.config?.apiBaseUrl}/auth/refresh-token`,
        {},
        {
          timeout: TIMEOUT,
          headers: {
